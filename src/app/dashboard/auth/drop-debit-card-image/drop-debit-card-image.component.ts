    ...
    reader.onload = async function() {
      ...
      if(response) {
        ...
        if (response.status === 'error')
        {
          ...
          {
            me.showErrorMessage('The dropped debit card image is not valid. Please ensure you are using the original unmodified debit card image, which can be modified if shared via messaging/file sharing apps.');
          }
          else
          {
